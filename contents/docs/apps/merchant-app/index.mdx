---
title: Fhish Merchant App
description: A dashboard for merchants to accept crypto payments and manage their business.
---

## Overview

The Fhish Merchant App provides an interface for businesses to integrate Fhish and monitor their financial operations.
...
Fhish uses a sophisticated settlement layer to bridge crypto payments and business account needs:
1.  **Payment Collection**: The `MerchantRouter.sol` handles incoming customer payments.
2.  **AMM Integration**: If the payment asset (e.g., WETH) differs from the merchant's target settlement asset (e.g., USDC), Fhish automatically swaps it via the internal AMM pools.
...
3.  **Integration**: Add the Fhish button or use the redirect URL.
